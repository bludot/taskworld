...
describe('Board', () => {
  ...
  it('Should create 10 ships', async () => {
    expect(instance.ships).toHaveLength(10);
  });
  it('should place ship', async () => {
    instance.placeShip(ShipType.Submarine, { x: 0, y: 0 }, { x: 3, y: 0 });
    expect(instance.ships.filter(ship => ship.placed)).toHaveLength(1)
  })
  ...
  it('should fail with all ships placed', async () => {
    instance.placeShip(ShipType.Battleship, {x: 2, y: 0}, {x:6, y:0});
    expect(() => {
      instance.placeShip(ShipType.Battleship, {x: 2, y: 0}, {x:6, y:0});
